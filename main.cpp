...
template <typename T> class DoubleBuffer{//用于实现双缓冲机制
private:
	T Resource1;//存储两个资源对象
	T Resource2;

	T *ReadQueue[2];//存储两个可读资源的指针
	T *WriteQueue[2];//存储两个可写资源的指针
	UBINT ReaderCount[2];//记录两个可读资源被读取的次数

	nsBasic::Mutex SyncMutex; //互斥锁，用于保护对象共享资源的并发访问
public:
	...
	void SubmitRead(T * const lpRes){//提交已读取的资源
		if (nullptr != lpRes){
			this->SyncMutex.Enter();
	...
	void SubmitWrite(T * const lpRes){//提交写完的资源
		if (nullptr != lpRes){
			this->SyncMutex.Enter();
...
extern int UserMain(){
	...
	ScreenRect.XMin = 20;//设置窗口大下
	ScreenRect.YMin = 20;
	ScreenRect.XMax = 820;
	ScreenRect.YMax = 620;
	nsGUI::LoadWindowMgr();

	nsGUI::GLWindow MyWnd(&ScreenRect);																			//创建了一个MyWnd的窗口对象
	nsBasic::SetMsgPort(&(nsBasic::GetThreadExtInfo()->MsgManager), (nsBasic::ObjGeneral *)&MyWnd, IO_VIDEO_OUT);//设置消息端口

	nsGUI::ControlManager MyCtrlMgr;		//创建控制管理器
	nsBasic::SetMsgPort(&(nsBasic::GetThreadExtInfo()->MsgManager), (nsBasic::ObjGeneral *)&MyCtrlMgr, IO_KEYBD);//设置消息端口
	...
	//nsAudio::OutputDevice_WASAPI MySndOutputDevice;				//输出设备
	nsAudio::OutputDevice_DS MySndOutputDevice;
